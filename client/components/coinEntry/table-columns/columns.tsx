...
import { CoinEntryModal, CoinModal } from "@/app/utils/modals";
...
export const columns: ColumnDef<CoinEntryModal>[] = [
  {
    accessorKey: "createdAt",
    header: ({ column }) => (
      <DataTableColumnHeader column={column} title="Entry Time" />
    ),
    cell: ({ row }) => <div className="w-auto ml-2 text-xs font-[100] text-muted-foreground hover:text-primary">{moment.utc(row.original.createdAt).local().format("HH:mm:ss")}</div>
  },
  {
    accessorKey: "code",
    header: ({ column }) => (
